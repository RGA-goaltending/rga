...
import { Loader, CheckCircle, AlertCircle, Lock, Calendar, Clock, Users, ArrowRight } from 'lucide-react';
...
export default function BookSession() {
  ...
  useEffect(() => {
    if (user && !isAdmin) {
      // Fetch ALL slots sorted by date (We filter locally to show Sold Out slots)
      const q = query(
        collection(db, "training_slots"), 
        orderBy("date", "asc") // Get closest dates first
      );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 font-sans selection:bg-[#D52B1E] selection:text-white">
      ...
      <div className="max-w-6xl mx-auto mb-12 border-b border-white/10 pb-6">
        ...
        <h1 className="text-4xl font-black tracking-tighter uppercase">Secure Your <span className="text-[#D52B1E]">Position</span></h1>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
             ...
             <p className="text-gray-500 italic bg-white/5 p-6 border border-white/10">No sessions available right now.</p>
          ) : (
            slots.map(slot => {
              // Logic: Is it full?
              ...
              return (
                <button 
                  ...
                  className={`
                    ...
                    ${isFull 
                       ...
                       : isSelected 
                          ? 'bg-[#D52B1E] border-[#D52B1E] shadow-[0_0_30px_rgba(213,43,30,0.3)]' 
                ...
                >
                  <div className="flex justify-between items-center relative z-10">
                    ...
                    {/* Date / Time */}
                    <div>
                      <div className={`text-xl font-black uppercase tracking-tighter ${isSelected ? 'text-white' : 'text-white'}`}>
                    ...
                    <div className="text-right">
                      <div className={`text-sm font-bold uppercase tracking-wide mb-1 ${isSelected ? 'text-white' : 'text-gray-300'}`}>
                        {slot.packageName}
                      </div>
        ...
        <div className="relative" ref={formRef}>
          <div className={`sticky top-8 transition-opacity duration-500 ${selectedSlot ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                ...
                <form onSubmit={handleBook} className="space-y-6 relative z-10">
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-black/50 border border-white/20 p-4 text-white focus:border-[#D52B1E] focus:outline-none transition-colors font-mono text-sm"
                    />
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-white text-black py-4 font-bold uppercase tracking-widest hover:bg-[#D52B1E] hover:text-white transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 mt-4 group"
                  >
                    {isSubmitting ? (
