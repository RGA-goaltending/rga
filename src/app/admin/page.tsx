...
// TYPES
interface TrainingSlot { id: string; date: string; startTime: string; packageName: string; price: number; capacity: number; bookedCount: number; status: string; }
interface PackageTier { id: string; name: string; price: number; price5: number; price10: number; peopleCount: number; maxQuantity: number; order: number; }
interface EventCamp { id: string; title: string; description: string; startDate: string; endDate: string; price: number; capacity: number; bookedCount: number; status: string; }
interface GalleryItem { id: string; imageUrl: string; description: string; createdAt: string; }
interface Booking { id: string; clientName: string; clientEmail: string; clientPhone: string; packageName: string; slotId: string; status: string; createdAt: string; }
...
export default function AdminDashboard() {
  ...
  // --- NEW: MULTI-SESSION HANDLER ---
  const handleAddDateRow = () => {
    setSessionDates([...sessionDates, { date: '', startTime: '' }]);
  };
  ...
  const handleDateChange = (index: number, field: 'date' | 'startTime', value: string) => {
    const newDates = [...sessionDates];
  ...
  const handleMultiSlotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Loop through all rows and create slots
  ...
  const handleUpdateStatus = async (bookingId: string, slotId: string, newStatus: string) => {
    ...
    try { const res = await fetch('/api/booking/status', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ bookingId, slotId, status: newStatus }) }); if (res.ok) setSuccessMsg("Updated!"); } catch (e) { alert("Error"); }
  };

  // Camp/Gallery handlers omitted for brevity (kept standard from previous code)
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 border-b border-white/10 pb-6 gap-6 w-full">
          <div className="w-full lg:w-auto">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
                <button onClick={() => setActiveTab('requests')} className={`flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase rounded ${activeTab === 'requests' ? 'bg-[#D52B1E] text-white' : 'bg-white/5 text-gray-400'}`}>Requests {bookings.filter(b => b.status === 'pending').length > 0 && <span className="bg-white text-black px-1.5 rounded-full">{bookings.filter(b => b.status === 'pending').length}</span>}</button>
                    ...
                    <table className="w-full text-left min-w-[700px]">
                        ...
                        <tbody className="divide-y divide-white/5 text-sm">{bookings.map(b => (
                            <tr key={b.id} className="hover:bg-white/5">
                                <td className="p-4"><div className="font-bold text-white">{b.clientName}</div><div className="text-xs text-gray-500">{b.clientPhone}</div></td>
                ...
                <div className="lg:col-span-1 bg-[#111] border border-white/10 p-6 rounded-xl">
                    ...
                    <form onSubmit={handleMultiSlotSubmit} className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase text-gray-500 font-bold">1. Select Package</label>
                            <select className="w-full bg-black/50 border border-white/20 p-3 text-white text-sm rounded focus:border-[#D52B1E] outline-none" value={selectedPackage} onChange={(e) => { const pkg = packages.find(p => p.name === e.target.value); setSelectedPackage(e.target.value); setSlotPrice(pkg ? pkg.price : 0); }}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[10px] uppercase text-gray-500 font-bold">Price</label><input type="number" className="w-full bg-black/50 border border-white/20 p-2 text-white text-sm rounded" value={slotPrice} onChange={e => setSlotPrice(Number(e.target.value))} /></div>
                            <div><label className="text-[10px] uppercase text-gray-500 font-bold">Slots</label><input type="number" className="w-full bg-black/50 border border-white/20 p-2 text-white text-sm rounded" value={slotCapacity} onChange={e => setSlotCapacity(Number(e.target.value))} /></div>
                        </div>

                        <div className="space-y-2 border-t border-white/10 pt-4">
                            <label className="text-[10px] uppercase text-gray-500 font-bold">2. Add Dates & Times</label>
                            {sessionDates.map((session, idx) => (
                                <div key={idx} className="flex gap-2 items-center animate-in fade-in slide-in-from-left-4">
                                    <input type="date" required className="bg-black/50 border border-white/20 p-2 text-white text-xs rounded w-1/2" value={session.date} onChange={e => handleDateChange(idx, 'date', e.target.value)} />
                                    <input type="time" required className="bg-black/50 border border-white/20 p-2 text-white text-xs rounded w-1/3" value={session.startTime} onChange={e => handleDateChange(idx, 'startTime', e.target.value)} />
                                    {idx > 0 && <button type="button" onClick={() => handleRemoveDateRow(idx)} className="text-red-500 hover:text-white"><Minus size={16}/></button>}
                                </div>
                            ))}
                            <button type="button" onClick={handleAddDateRow} className="text-xs text-[#D52B1E] font-bold uppercase hover:text-white flex items-center gap-1 mt-2"><Plus size={14}/> Add Another Date</button>
                        </div>
                ...
                <div className="lg:col-span-2 bg-[#111] border border-white/10 rounded-xl overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left min-w-[500px]">
                            <thead className="bg-white/5 border-b border-white/10"><tr><th className="p-4 text-[10px] text-gray-400 uppercase">Date</th><th className="p-4 text-[10px] text-gray-400 uppercase">Package</th><th className="p-4 text-[10px] text-gray-400 uppercase">Status</th><th className="p-4 text-right"></th></tr></thead>
                            <tbody className="divide-y divide-white/5 text-sm">{slots.map(s => (
                                <tr key={s.id} className="hover:bg-white/5">
                                    <td className="p-4 font-mono">{s.date} <span className="text-gray-500">@</span> {s.startTime}</td>
                                    <td className="p-4 font-bold uppercase">{s.packageName}</td>
                                    <td className="p-4 text-xs font-mono uppercase">{s.status === 'requested' ? <span className="text-yellow-500">Requested</span> : <span className="text-green-500">Available</span>}</td>
                                    <td className="p-4 text-right"><button onClick={() => handleDelete('training_slots', s.id)} className="text-gray-600 hover:text-[#D52B1E]"><Trash2 size={16}/></button></td>
                                </tr>
                            ))}</tbody>
                        </table>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-[#111] border border-white/10 p-6 rounded-xl">
                    ...
                    <form onSubmit={handlePackageSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <input type="number" placeholder="Price" required className="bg-black/50 border border-white/20 p-3 text-white text-sm rounded" value={pkgForm.price || ''} onChange={e => setPkgForm({...pkgForm, price: Number(e.target.value)})} />
                            <select className="bg-black/50 border border-white/20 p-3 text-white text-sm rounded" onChange={e => setIsGroup(e.target.value === 'group')}><option value="private">Private</option><option value="group">Group</option></select>
                        </div>
                ...
                <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-white/5 border-b border-white/10"><tr><th className="p-4 text-[10px] text-gray-400 uppercase">Package</th><th className="p-4 text-right">Action</th></tr></thead>
                        <tbody>{packages.map(p => (<tr key={p.id} className="border-b border-white/5"><td className="p-4 text-white font-bold">{p.name} - ${p.price}</td><td className="p-4 text-right"><button onClick={() => handleDelete('packages', p.id)}><Trash2 size={16} className="text-gray-500 hover:text-white"/></button></td></tr>))}</tbody>
                    </table>
                </div>
            </div>
        )}
