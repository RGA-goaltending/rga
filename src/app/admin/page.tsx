...
import { packages } from '../lib/packages'; // Import packages
...
export default function AdminDashboard() {
  ...
  const handleAddSlot = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError(null);
    if (!date || !startTime || !packageName) {
      setFormError('Date, time, and package cannot be empty.');
      return;
    }
    ...
    if (!selectedPackage) {
      setFormError('Invalid package selected.');
      ...
      <div className="bg-slate-900 text-white p-6 rounded-xl shadow-lg mb-12">
        ...
        <form onSubmit={handleAddSlot} className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <select value={packageName} onChange={e => setPackageName(e.target.value)} className="w-full p-2 rounded bg-slate-800 border border-slate-700 focus:border-red-500 outline-none" disabled={isSubmitting}>
              {packages.map(p => (
      ...
      <div className="bg-white border rounded-lg overflow-hidden shadow-sm">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y">
            {slotsLoading ? (
              <tr><td colSpan={6} className="text-center p-8"><Loader className="animate-spin inline-block" /> Loading slots...</td></tr>
            ) : slots.length === 0 ? (
              <tr><td colSpan={6} className="text-center p-8 text-slate-500">No training slots available.</td></tr>
            ) : (
              slots.map(slot => (
                <tr key={slot.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">{slot.packageName}</td>
                  <td className="p-4">
