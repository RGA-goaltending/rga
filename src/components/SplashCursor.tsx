...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Helper function to return color based on vertical position (0 to 1)
    // 0 is bottom, 1 is top
    function getRussianFlagColor(yPosition: number): ColorRGB {
        // Top Band: White (y > 0.66)
        if (yPosition > 0.66) {
            return { r: 0.2, g: 0.2, b: 0.2 }; // Scaled down white for bloom effect
        } 
        // Middle Band: Blue (0.33 < y <= 0.66)
        else if (yPosition > 0.33) {
            return { r: 0.0, g: 0.1, b: 0.5 }; // Deep Blue
        } 
        // Bottom Band: Red (y <= 0.33)
        else {
    ...
    function updateColors(dt: number) {
      // Disabled random color cycling so flag colors persist
      /*
    ...
    function clickSplat(pointer: Pointer) {
      ...
      // We use the pointer's own color, which is already set by the flag logic in UpdatePointerDownData
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
      ...
      pointer.deltaY = 0;
      // Use Flag Logic
      pointer.color = getRussianFlagColor(pointer.texcoordY);
    }
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: ColorRGB) {
      ...
      // Use Flag Logic (ignore incoming color arg and recalculate based on new Y)
      pointer.color = getRussianFlagColor(pointer.texcoordY);
    }
    ...
    function handleFirstMouseMove(e: MouseEvent) {
      ...
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
      // Calculate initial flag color
      const yNorm = 1 - posY / canvas!.height;
      const color = getRussianFlagColor(yNorm);
